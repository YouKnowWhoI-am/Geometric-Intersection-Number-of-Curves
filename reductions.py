...
# Take a note of changes like: dlist.change_turn(node.next.next, n + 1), at the beginning of most functions.
...
def std_spur(dlist, node):
    ...
    dlist.change_turn(node.next, node.turn + node.next.next.turn)
...
def std_rt_bracket(dlist, node, n):
    ...
    dlist.change_turn(node.next.next, n + 1)  
    dlist.decrease_run_length_by(node.next.next.next, 1)
    dlist.decrease_run_length_by(node.next, 1)
    dlist.change_turn(node, node.turn + 1)
    dlist.change_turn(node.next.next, node.next.next.turn + 1)
    dlist.change_turn(node.next, 2) 

def cyclic_rt_bracket(dlist, node, n):
    # (\vbar{1}, \vbar{2}^r) --> (3, 2^{r - 2})
    dlist.change_turn(node, n + 1) 
    dlist.decrease_run_length_by(node.next.next, 2)
    dlist.change_turn(node.next, 2)
    dlist.change_turn(node, 3)
...
def near_cyclic_rt_bracket(dlist, node, n):
    ...
    dlist.decrease_run_length_by(node.next.next.next, 1)
    dlist.decrease_run_length_by(node.next, 1)
    dlist.change_turn(node, node.turn + 2)
    dlist.change_turn(node.next, 2)
...
def std_lt_bracket(dlist, node, n):
    ...
    dlist.change_turn(node, node.turn - 1)
    dlist.change_turn(node.next.next, node.next.next.turn - 1)
    dlist.change_turn(node.next, n - 2)
...
def rt_shift_1(dlist, node, n):
    ...
    dlist.change_turn(node.next, n + 1) 
    dlist.change_turn(node.next.next.next, n + 1) 
    dlist.change_turn(node.next.next, 3)
    dlist.change_turn(node.turn, node.turn + 1)
    dlist.change_turn(node.next.next.next.next.turn, node.next.next.next.next.turn + 1)
    dlist.insertAfter(node, 1, 1)
    dlist.insertAfter(node.next.next.next.next, 1, 1)
    dlist.change_turn(node.next.next, 2)
    dlist.change_turn(node.next.next.next.next, 2)
    dlist.decrease_run_length_by(node.next.next, 1)
...
def rt_shift_2(dlist, node, n):
    ...
    dlist.change_turn(node.next.next, n + 1)
    dlist.change_turn(node.next.turn, n + 2)
    dlist.change_turn(node.turn, node.turn + 1)
    dlist.change_turn(node.next.next.next.turn, node.next.next.next.turn + 1)
    ...
    dlist.change_turn(node.next, 2)
    dlist.change_turn(node.next.next, 1)
...
def rt_shift_4(dlist, node, n):
    ...
    dlist.change_turn(node.next.next.next, n + 1) 
    dlist.change_turn(node.next.next, 3)
    dlist.change_turn(node.turn, node.turn + 2)
    dlist.insertAfter(node, 1, 1)
    dlist.insertAfter(node.next.next.next.next, 1, 1)
    dlist.change_turn(node.next.next, 2)
    dlist.change_turn(node.next.next.next.next, 2)
...
def rt_shift_7(dlist, node, n):
    ...
    dlist.change_turn(node.next.next.next, n + 1)
    dlist.change_turn(node.next.next, 3)
    dlist.change_turn(node.turn, 1)
    dlist.change_turn(node.next, 2)
    dlist.change_turn(node.next.next.next, 2)

def rt_shift_8(dlist, node, n):
    # (\vbar{2}^l) --> (2^{l})
    dlist.change_turn(node, 2)